...
otu_count <- read_tsv("final.opti_mcc.0.03.subsample.oct.24.bacterial.shared") %>%
...
tree <- read_tree("final.opti_mcc.0.03.rep.phylip.oct.24.bacterial.tre") #representative tree make using get.oturep in mothur
...
write.csv(plot_data, file = "group.distance.oct.24.csv", row.names = FALSE)
##

beta_NTI_ave <- read.csv("group.distance.oct.24.csv") %>%
  rename_all(tolower) %>%
  group_by(combo) %>%
