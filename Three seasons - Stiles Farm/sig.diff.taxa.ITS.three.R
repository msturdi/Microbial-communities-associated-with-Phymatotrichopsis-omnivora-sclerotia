...
shared <- read_tsv("final.opti_mcc.0.03.subsample.three.fungal.shared",
...
taxonomy <- read_tsv(file="final.opti_mcc.0.03.cons.three.fungal.taxonomy") %>%
  select("OTU", "Taxonomy") %>%
  rename_all(tolower) %>%
...
metadata <- read_excel("metadata.three.fungal.xlsx") %>%
  rename(group = sample) %>%
...
composite %>%
  ...
  labs(x= "Relative abundance", y=NULL,
       title="Significantly Different Orders (Fungal)<br>Three Seasons at the Stiles Farm") +
  theme_light() +
...
ggsave("sig.diff.orders.ITS.three.png", width=13, height=13)
