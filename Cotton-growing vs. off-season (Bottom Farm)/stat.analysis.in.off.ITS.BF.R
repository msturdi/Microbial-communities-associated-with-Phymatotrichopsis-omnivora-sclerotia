...
metadata <- read_excel(path="metadata.ITS.in.off.BF.xlsx") %>%
  mutate(combo = glue("{season} {treatment}"))

alpha <- read_tsv(file="final.opti_mcc.groups.ave-std.fungal.BF.summary", 
...
t.test(sobs~season, data=meta_alpha, #not sig.
       var.equal=TRUE,
       conf.level=0.95)
...
meta_alpha %>%
  ...
  labs(x=NULL,
       ...
       title="Cotton-growing vs. Off Season Fungal Diversity<br>**Bottom Farm (Non-conducive)**") +
  theme_classic() +
...
ggsave("shannon.ITS.in.off.BF.png", width=9, height=7)
...
meta_alpha %>%
  ...
  labs(x=NULL,
       ...
       title="Comparison of In- vs. Off-season Fungal Observed Diversity<br>Bottom Farm") +
  theme_classic() +
...
ggsave("sobs.ITS.in.off.BF.png", width=8, height=6)
...
meta_alpha %>%
  ...
  labs(x=NULL,
       y="Shannon Evenness",
       title="Comparison of In- vs. Off-season Fungal Evenness<br>Bottom Farm") +
  theme_classic() +
  theme(axis.text.x = element_markdown(size=10),
        axis.text.y = element_markdown(size=10),
        legend.position = "none",
...
ggsave("even.ITS.in.off.BF.png", width=8, height=6)
...
meta_alpha %>%
  ...
  labs(x=NULL,
       y="Shannon",
       title="Comparison of In- vs. Off-season Fungal Diversity<br>Bottom Farm") +
  theme_classic() +
  theme(axis.text.x = element_markdown(size=10),
        axis.text.y = element_markdown(size=10),
        legend.position = "none",
...
ggsave("shannon.ITS.in.off.BF.trt.png", width=8, height=6)
...
location_facet <- meta_alpha_long %>%
  ...
  labs(x=NULL,
       y=NULL,
       title="In- vs. Off-season Additional Fungal Diversity Indices<br>Bottom Farm") +
  theme_light() +
  theme(axis.text.x = element_markdown(size=16),
        axis.text.y = element_markdown(size=12),
...
ggsave("other.metrics.fungal.in.off.BF.location.png", width=14, height=8)
