...
metadata <- read_excel(path="metadata.in.off.24.16S.BF.xlsx") %>%
  mutate(combo = glue("{season} {treatment}"))

alpha <- read_tsv(file="final.opti_mcc.groups.ave-std.bacterial.BF.summary", 
...
season_shannon <- meta_alpha %>%
  ...
  scale_x_discrete(limits=c("in", "off"),
                   labels=c("Cotton-growing<br>season", "Off-season")) +
  scale_y_continuous(limits=c(6.5,6.95)) +
  labs(x=NULL,
       y="Bacterial Shannon Diversity",
       title="Cotton-growing vs. Off Season Bacterial Diversity<br>**Bottom Farm (Non-conducive)**") +
  theme_classic() +
...
season_shannon +
  geom_text(data=tibble(x=2, y=6.95), 
            aes(x=x, y=y, label="*"), size=9,
            inherit.aes = FALSE) 

ggsave("shannon.16S.in.off.BF.png", width=9, height=7)
...
meta_alpha %>%
  ...
  labs(x=NULL,
       ...
       title="Comparison of In- vs. Off-season Bacterial Observed Diversity<br>Bottom Farm") +
  theme_classic() +
...
ggsave("sobs.16S.in.off.BF.png", width=8, height=6)
...
meta_alpha %>%
  ...
  labs(x=NULL,
       y="Shannon Evenness",
       title="Comparison of In- vs. Off-season Bacterial Evenness<br>Bottom Farm") +
  theme_classic() +
  theme(axis.text.x = element_markdown(size=10),
        axis.text.y = element_markdown(size=10),
        legend.position = "none",
...
ggsave("even.16S.in.off.BF.png", width=8, height=6)
...
meta_alpha %>%
  ...
  labs(x=NULL,
       y="Shannon",
       title="Comparison of In- vs. Off-season Bacterial Diversity<br>Bottom Farm") +
  theme_classic() +
  theme(axis.text.x = element_markdown(size=10),
        axis.text.y = element_markdown(size=10),
        legend.position = "none",
...
ggsave("shannon.16S.in.off.BF.trt.png", width=8, height=6)
...
location_facet <- meta_alpha_long %>%
  ...
  labs(x=NULL,
       y=NULL,
       title="In- vs. Off-season Additional Bacterial Diversity Indices<br>Bottom Farm") +
  theme_light() +
  theme(axis.text.x = element_markdown(size=16),
        axis.text.y = element_markdown(size=12),
...
ggsave("other.metrics.16S.in.off.BF.location.png", width=14, height=8)
