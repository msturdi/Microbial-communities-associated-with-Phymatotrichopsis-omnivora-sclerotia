...
shared_file <- read_tsv("final.opti_mcc.0.03.subsample.bacterial.BF.shared")
...
read_tsv(in_off) %>%
  ...
  labs(y=NULL, x="LDA Score(log 10)",
       title="LDA Plot (LDA > 2.5) In- vs. Off-season Bacterial<br>Bottom Farm") +
...
ggsave("lefse.in.off.16S.season.BF.png", width=12, height=22)
...
ggsave("lefse.ilive.inone.16S.BF.png", width=10, height=8)
...
ggsave("lefse.idead.inone.16S.BF.png", width=10, height=8)
...
read_tsv(olive_onone) %>%
  ...
  geom_col() +
  labs(y=NULL, x="LDA Score(log 10)",
       title="LDA Plot (LDA > 2.5) Off-season Bacterial<br>Bottom Farm") +
  scale_x_continuous(expand = c(0,0), limits = c(0, 4), breaks = seq(0, 4, by=2)) +
  scale_fill_manual(name=NULL, 
...
ggsave("lefse.olive.onone.16S.BF.png", width=12, height=15)
...
ggsave("lefse.odead.onone.16S.BF.png", width=12, height=12)
...
ggsave("lefse.ilive.olive.16S.BF.png", width=12, height=22)
...
ggsave("lefse.idead.odead.16S.BF.png", width=12, height=22)
