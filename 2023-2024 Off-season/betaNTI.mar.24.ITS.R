...
otu_count <- read_tsv("final.opti_mcc.0.03.subsample.mar.24.fungal.shared") %>%
  select(Group, starts_with("Otu")) %>%
  rename(sample = Group) %>%
  column_to_rownames(var = "sample")
...
#tree <- read_tree("final.phylip.oct.24.fungal.tre") #ran into issue here with tip labels in tree not matching the OTU labels in the other objects
tree <- read_tree("final.opti_mcc.0.03.rep.phylip.mar.24.tre") #representative tree make using get.oturep in mothur
...
write.csv(plot_data, file = "group.distance.mar.24.ITS.csv", row.names = FALSE)

beta_NTI_ave <- read.csv("group.distance.mar.24.ITS.csv") %>%
  rename_all(tolower) %>%
  group_by(combo) %>%
...
g2 = g1 +
  ...
  labs(x=NULL,
       y="betaNTI",
       title="Stochastic or Deterministic Selection<br>Off Season 2023-2024 **Fungal**") +
  theme_classic() +
...
ggsave("betaNTI.mar24.ITS.png", width=12, height=8)
...
proportions %>%
  ...
  labs(x=NULL,
       ...
       title="Proportion of Stochastic vs. Deterministic Selection<br>Off Season 2023-2024 **Fungal**") +
  theme_classic() +
  theme(axis.text.x = element_markdown(size = 14, color = "black"),
