...
shared_file <- read_tsv("final.opti_mcc.0.03.subsample.mar.24.bact.shared")
...
read_tsv(bf_stiles) %>%
  ...
  #mutate(LDA = if_else(Class == "BF", -1 * LDA, LDA),
  #taxon = fct_reorder(taxon, LDA)) %>%
  ggplot(aes(x=LDA, y=taxon, fill=Class)) +
  geom_col() +
  labs(y=NULL, x="LDA Score(log 10)",
       title="LDA Plot (LDA > 2.5) Off-season Bacterial<br>Bottom Farm vs. Stiles Farm") +
...
ggsave("lefse.slive.snone.16S.png", width=12, height=22)
...
ggsave("lefse.sdead.snone.16S.png", width=12, height=22)
...
read_tsv(blive_slive) %>%
  ...
  geom_col() +
  labs(y=NULL, x="LDA Score(log 10)",
       title="LDA Plot (LDA > 2.5) Off-season Bacterial<br>Bottom Farm Live vs. Stiles Farm Live") +
  scale_x_continuous(expand = c(0,0), limits = c(0, 4.2), breaks = seq(0, 4.2, by=2)) +
  scale_fill_manual(name=NULL, 
...
read_tsv(bdead_sdead) %>%
  ...
  geom_col() +
  labs(y=NULL, x="LDA Score(log 10)",
       title="LDA Plot (LDA > 2.5) Off-season Bacterial<br>Bottom Farm Dead vs. Stiles Farm Dead") +
  scale_x_continuous(expand = c(0,0), limits = c(0, 4), breaks = seq(0, 4, by=2)) +
  scale_fill_manual(name=NULL, 
